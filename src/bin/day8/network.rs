...
impl Network<'_> {
    ...
    pub(crate) fn par_iter<'a>(&'a self, start: &'a [&'a str], turns: impl Iterator<Item=char>) -> ParNetworkIter<'a, impl Iterator<Item=char>> {
...
impl<'a, I> Iterator for NetworkIter<'a, I> where I: Iterator<Item=char> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.turns.next() {
            Some('L') => self.net.get(self.start).map(|(l,_)| *l),
            Some('R') => self.net.get(self.start).map(|(_,r)| *r),
            _ => unreachable!()
        }.map(|next| {
            self.start = next;
            next
